...
## Модальное сопоставление

Форма `match μ x (...)` проверяет разбираемое выражение `x` под
модальностью `μ` и использует элиминатор, сгенерированный для этой модальности.
Тип `D` без конструкторов путей порождает `ind-D` и `rec-D` для тождественной
модальности, а также `ind-op-D`/`rec-op-D`, `ind-b-D`/`rec-b-D` и
...
Если поле конструктора объявлено под модальностью `ν`, его биндер в модальном
элиминаторе и соответствующая переменная ветви получают `μ · ν`.
Сгенерированные индуктивные гипотезы остаются обычными; `_` в примере — такая
неиспользуемая гипотеза. Параметры типа данных, индексы, shape-поля и их guards
также наследуют `μ`.

Форма с явным мотивом записывается `match μ x into motive (...)`. Для
неиндексированного типа `D` мотив имеет тип `(z :_μ D) → U`. Для
индексированного типа каждая связывающая переменная телескопа мотива наследует
`μ`, поэтому для одного обычного индекса мотив имеет тип
`(i :_μ I) → (z :_μ D i) → U`. Высшие индуктивные типы пока порождают только
`ind-D` и `rec-D`; модальное сопоставление по ним отклоняется, поскольку для
него нужна модальная индукция по путям.

Поскольку модальность сразу после `match` выбирает эту форму,
для обычного `match` по выражению с модальным префиксом нужны скобки:
`match (mod ♭ x) (...)`.
...
На конструкторе пути ничего не вычисляется дефиниционально; пропозициональная замена — порождённые правила `compute-` (см. [`#data`](commands/data.rzk.md)).
