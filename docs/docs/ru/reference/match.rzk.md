...
модальностью `μ`. Если метод конструктора связывает аргумент под
модальностью `ν`, соответствующая переменная ветви связывается под
композицией `μ · ν`. Это правило применяется ко всем полям и индуктивным
гипотезам. Обычный `match x (...)` — частный случай `μ = _id`.
...
Здесь обе переменные ветви доступны под `_b`: их модальности равны
`_b · _id = _b` и `_b · _op = _b`. Форма с явным мотивом записывается
`match μ x into motive (...)`. Для неиндексированного типа `D` мотив имеет тип
`(z :_μ D) → U`: связывающая переменная разбираемого значения наследует
модальность match. Для индексированного типа каждая связывающая переменная
телескопа мотива наследует `μ`, поэтому для одного
обычного индекса мотив имеет тип `(i :_μ I) → (z :_μ D i) → U`.
