...
### Скрытие терма-доказательства

!!! info "Начиная с версии `0.9.0`"

Иногда мы хотим показать только _форму_, которую населяет терм, с её граничными
рёбрами и гранями, но без заполняющего её терма-доказательства. Мы устанавливаем
опцию `#!rzk "render-hide-term"` в `#!rzk "yes"` (по умолчанию `#!rzk "no"`):

```rzk
#set-option "render-hide-term" = "yes"
```

При скрытом терме-доказательстве внутренняя метка убирается, и остаются только
заданные граничные метки. Тогда диаграмма показывает клетку, которую нужно
заполнить, а не конкретное заполнение:
...
```rzk
#unset-option "render-hide-term"  -- вернуть значение по умолчанию
```

Тот же механизм использует запрос типизированных дырок, чтобы нарисовать
клетку-цель дырки. Он показывает форму, которую дырка должна населить, с её
заданными рёбрами и со скрытым термом-доказательством.
