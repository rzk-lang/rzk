...
Модальное let-связывание записывается в форме `#!rzk let ext mod inn x := value in body`, где:

- `#!rzk value` проверяется на тип `#!rzk inn T` под **`ext`-замком**
- `#!rzk x` \(:^{ext \cdot inn}\) `#!rzk T` — это терм, извлечённый из-под модальности `#!rzk inn`, и с ним проверяется `#!rzk body`

Если `#!rzk ext` опущен, то `#!rzk let mod m x := value in body` — это сахар для `#!rzk let _id mod m x := value in body`.

Есть также одномодальная форма `#!rzk let m x := t in body` — это сахар для `#!rzk let mod m x := mod m t in body`. В отличие от `#!rzk let mod`, она ничего не извлекает из модального значения, а лишь проверяет, что `#!rzk t` доступен под модальностью `#!rzk m`, и связывает `#!rzk x` \(:^{m}\) с ним.

Ограничение может уточнять модальный тип целиком, например
`#!rzk x : (♭ A)[φ ↦ mod ♭ a]`. Такое значение устраняется той же формой
`#!rzk let mod ♭ y := x in ...`: внешнее ограничение не скрывает тип модального содержимого, а его граничное уравнение остаётся доступным через обычные правила ограничений.
...
В такой записи `#!rzk body` проверяется на тот тип, который ожидается от всего
let-связывания, поэтому цель не может зависеть от устраняемого значения. Явный
мотив, записанный после `#!rzk into`, снимает это ограничение. Мотив — это то
*семейство*, в которое устраняет let-связывание, как функция от модального
значения:

- `#!rzk motive` проверяется на тип `#!rzk (z` \(:^{ext}\) `#!rzk inn T) → U`
- `#!rzk body` проверяется на тип `#!rzk motive (mod inn x)`
- само let-связывание имеет тип `#!rzk motive value`

Это правило зависимого устранения из мультимодальной теории типов, и именно оно
позволяет модальному let-связыванию доказывать утверждение об устраняемом
значении. Оно нужно всякий раз, когда цель упоминает это значение. У `#!rzk ♭` нет η-правила,
так что без мотива ничто не отождествляет `#!rzk x` с `#!rzk mod ♭ a`, и принцип
индукции ниже проходит только с мотивом:
...
Мотив можно записать и λ-абстракцией, например `#!rzk into (\ z → C z)`.
Отметим, что пока в качестве области значений поддерживается только `#!rzk U`:
мотив со значениями в `#!rzk CUBE` или `#!rzk TOPE` записать нельзя.

## Модальные связки

Модальные аннотации параметров `#!rzk (x :µ A)` связывают переменную `#!rzk x` непосредственно под модальностью `#!rzk µ` с типом `#!rzk A`. Это полноценная модальная связка — переменная `#!rzk x` доступна в соответствии с правилами коэрсии модальности `#!rzk µ`. Модальные связки доступны в `#!rzk λ`-абстракциях, `#!rzk Π`- и `#!rzk Σ`-типах, а также в списках аргументов определений.
...
- `#!rzk let ext mod inn x := value in body` вводит `#!rzk x` как **связку, параметризованную модальностью** с модальностью \(ext \cdot inn\).
