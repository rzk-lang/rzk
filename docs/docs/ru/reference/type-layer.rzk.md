...
Типы функций (зависимых произведений) \(\prod\_{x : A} B\) записываются как `#!rzk (x : A) -> B x`. Значения типов функций — это \(\lambda\)-абстракции, записанные одним из следующих способов:
...
Тип зависимой суммы \(\sum\_{x : A} B\) записывается как `#!rzk ∑ (x : A), B` или `#!rzk Sigma (x : A), B`. Значения типов зависимых сумм — это пары, записанные как `#!rzk (x, y)`.
...
- для
  - любого типа \(A\) и \(a : A\),
  - семейства типов \(C : \prod\_{x : A} ((a =\_A x) \to \mathcal{U})\) и
