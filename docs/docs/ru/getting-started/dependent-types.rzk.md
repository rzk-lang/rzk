...
В Rzk, пустой тип и типы-копроизведения объявляются как индуктивные типы
с помощью команды `#!rzk #data`.

### Пустой тип

Пустой тип — это индуктивный тип без конструкторов:

```rzk
#data Void
```

Объявление порождает принцип индукции `#!rzk ind-Void`
и его простую (не зависимую) версию `#!rzk rec-Void`:
...
Поскольку в пустом типе `#!rzk Void` нет элементов,
принцип индукции соответсвует принципу абсурности (из лжи следует что угодно).

### Тип-копроизведение

Тип-копроизведение — это индуктивный тип с двумя конструкторами —
один вводит терм из типа `#!rzk A`, другой из типа `#!rzk B`:
...
Чтобы устранить копроизведение, порождённый принцип индукции
требует два обработчика —
...
Правила вычисления _определительные_: `#!rzk ind-coprod A B C l r (inl A B a)`
вычисляется в `#!rzk l a` автоматически.
Например, следующее равенство доказывается через `#!rzk refl`:
...
Принцип рекурсии для копроизведений — это порождённая не зависимая версия:
...
Булев тип — это индуктивный тип с двумя конструкторами без полей:

```rzk
#data Bool := false | true
```

Порождённые принципы индукции и рекурсии имеют ожидаемые типы:
...
Поскольку правила вычисления индуктивных типов определительные,
`#!rzk not (not false)` вычисляется в `#!rzk false`, а `#!rzk not (not true)` — в `#!rzk true`,
и следующее доказательство проходит индукцией с двумя случаями `#!rzk refl`:
...
!!! warning "Индуктивные типы и симплициальная структура"

    Индуктивный тип приходит ровно со своим принципом индукции.
    Как тип взаимодействует с _симплициальной_ структурой Rzk — отдельный вопрос,
    и, вообще говоря, это взаимодействие невыводимо.
    В частности, принцип индукции `#!rzk Bool` не доказывает дискретность `#!rzk Bool`:
    дискретность `#!rzk Bool` эквивалентна предположению, что существует тип
    с двумя точками, не соединёнными стрелкой, а без дополнительных аксиом
    теория не может доказать это предположение.
    Таким образом, чтобы `#!rzk Bool` был дискретным, всё ещё необходима аксиома несвязности;
    объявление `#!rzk #data Bool` не доказывает ничего нового о `#!rzk 2 → Bool`.
