...
Function (dependent product) types \(\prod\_{x : A} B\) are written `#!rzk (x : A) -> B x`. Values of function types are \(\lambda\)-abstractions written in one of the following ways:
...
Dependent sum type \(\sum\_{x : A} B\) is written `#!rzk ∑ (x : A), B` or `#!rzk Sigma (x : A), B`. Values of dependent sum types are pairs written as `#!rzk (x, y)`.
...
- for
  - any type \(A\) and \(a : A\),
  - type family \(C : \prod\_{x : A} ((a =\_A x) \to \mathcal{U})\) and
