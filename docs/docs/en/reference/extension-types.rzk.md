...
4. Extension types \(\left\langle \prod_{t : I \mid \psi} A \vert ^{\phi} _{a} \right\rangle\) are written as `#!rzk (t : I | psi t) -> A [ phi |-> a ]`
   ...
   - specifying `#!rzk psi` in `#!rzk (t : I | psi t)` is mandatory;
...
5. Types of functions from a shape \(\prod_{t : I \mid \psi} A\) are a specialised variant of extension types and are written `#!rzk (t : I | psi t) -> A`
   - specifying the name of the argument is mandatory; i.e. `#!rzk (I | psi t) -> A` is invalid syntax!
   - values of function types are \(\lambda\)-abstractions written in one of the following ways:
     - `#!rzk \t -> <body>` — this is usually fine;
     - `#!rzk \(t : I | psi t) -> <body>` — this sometimes helps the typechecker;
