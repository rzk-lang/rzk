...
- every binder of a `#data` is checked against its layer, on the elaborated form, so the rule does not depend on the spelling. A *parameter* may be a type, `#!rzk CUBE`, or a tope family `#!rzk I → TOPE` — the parameter kinds of the meta-theoretic parameter layer, which the generic realisation below depends on. An *index* must be a type, full stop: cube points, shape points, `#!rzk CUBE`, and tope families are all rejected (a family varying over a shape is what extension types are for). A *field* may be a type, a cube point, or a shape point, and nothing else: a field of `#!rzk CUBE` or of a tope family would store a cube- or tope-layer citizen inside type-layer data. The modal tope kinds (`#!rzk _op TOPE` and families into it) are rejected in every position;
