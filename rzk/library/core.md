# Core definitions, functions and theorems
...
bool : U
  := (A : (_ : bool') -> U) -> (f : A false')


rec_bool : (A : U) -> (_ : A) -> (_ : A) -> (x : bool) -> A
  := \(A : U) -> \(f : A) -> \(t : A) -> x

ind_bool : (A : (_ : bool) -> U) -> (_ : A false) -> (_ : A true) -> (x : bool) -> A x
  := \(A : (_ : bool) -> U) -> \(f : A false) -> \(t : A true) -> \(x : bool) -> x (A x) f t
```
