...
Δ² : (t : 2 * 2) -> TOPE
  := \(t : 2 * 2) -> (second t) <= (first t)

Δ³ : (t : 2 * 2 * 2) -> TOPE
  := \(t : 2 * 2 * 2) -> (second t) <= (second (first t)) /\ (second (first t)) <= (first (first t))

shapeProd : (I : CUBE) -> (J : CUBE) -> (psi : (t : I) -> TOPE) -> (chi : (s : J) -> TOPE) -> (ts : I * J) -> TOPE
  := \(I : CUBE) -> \(J : CUBE) -> \(psi : (t : I) -> TOPE) -> \(chi : (s : J) -> TOPE) -> \(ts : I * J) -> psi (first ts) /\ chi (second ts)
...
Δ²-is-retract-of-Δ¹×Δ¹ : shapeRetract (2 * 2) Δ² Δ¹×Δ¹
  := \(A : U) -> (\(k : <{t : 2 * 2 | Δ¹×Δ¹ t} -> A [BOT |-> recBOT]>) -> \{ts : 2 * 2 | Δ² ts} -> k (first ts, second ts), (\(k : <{ts : 2 * 2 | Δ² ts} -> A [BOT |-> recBOT]>) -> \{ts : 2 * 2 | Δ¹×Δ¹ ts} -> recOR((first ts) <= (second ts), (second ts) <= (first ts), k (first ts, second ts), k (first ts, second ts)), \(k : <{ts : 2 * 2 | Δ² ts} -> A [BOT |-> recBOT]>) -> refl_{k : <{ts : 2 * 2 | Δ² ts} -> A [BOT |-> recBOT]>}))
```

```rzk
Δ³-is-retract-of-Δ²×Δ¹ : shapeRetract (2 * 2 * 2) Δ³ Δ²×Δ¹
  := \(A : U) -> (\(k : <{ts : 2 * 2 * 2 | Δ²×Δ¹ ts} -> A [BOT |-> recBOT]>) -> \{ts : 2 * 2 * 2 | Δ³ ts} -> k ((first (first ts), second ts), second (first ts)), (\(k : <{ts : 2 * 2 * 2 | Δ³ ts } -> A [BOT |-> recBOT]>) -> \{ts : 2 * 2 * 2 | Δ²×Δ¹ ts} -> recOR((second ts) <= (second (first ts)), (second (first ts)) <= (second ts) /\ (second ts) <= (first (first ts)) \/ (first (first ts)) <= second ts, k ((first (first ts), second ts), second (first ts)), recOR((second (first ts)) <= (second ts) /\ (second ts) <= (first (first ts)), (first (first ts)) <= second ts, k ((first (first ts), second ts), second (first ts)), k ((first (first ts), first (first ts)), second (first ts)))), \(k : <{ts : 2 * 2 * 2 | Δ²×Δ¹ ts} -> A[BOT |-> recBOT]>) -> refl_{k : <{ts : 2 * 2 * 2 | Δ²×Δ¹ ts} -> A[BOT |-> recBOT]>}))
```
