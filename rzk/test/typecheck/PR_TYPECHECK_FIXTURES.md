...
`warn-rstt-shape-dependency` covers outer points in domains and boundaries, distinct cubes, product projections, nested lambdas and local aliases. `ill-rstt-shape-boundary` and `ill-rstt-shape-domain` cover error mode; `happy-rstt-shape-dependency-off` leaves the new check disabled independently of standalone restriction warnings.
